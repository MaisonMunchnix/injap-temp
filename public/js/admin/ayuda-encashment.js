...
$(document).ready(function () {
    ...
    $('#btn-approve').click(function () {
        ...
        if (amount_requested == '' || amount_requested == 0 || amount_approved == '' || amount_approved == 0) {
            ...
            if (get_amt_request == amount_requested) {
                if (parseInt(amount_approved) > parseInt(amount_requested)) {
                    swal({
                        title: "Warning!",
    ...
    $('#claim_modal').on('shown.bs.modal', function (e) {
        $.ajax({
            url: '../get-encashment-data/' + claim_tid,
            type: 'GET',
            beforeSend: function () {
            ...
            success: function (data) {
                ...
                var data_tax = parseFloat(data_amount_appr) * encashment_tax;
                var total_claim = parseFloat(data_amount_appr) - parseFloat(data_tax) - processing_fee;
                $('#details_ar').text(addComma(data_amount_req) + 'PHP');
                $('#details_appr').text(addComma(data_amount_appr) + 'PHP');
                $('#details_tax').text(addComma(data_tax) + 'PHP');
    ...
    $('#view_encashment_modal').on('shown.bs.modal', function (e) {
        $.ajax({
            url: '../get-encashment-data/' + view_tid,
            type: 'GET',
            beforeSend: function () {
                console.log('Getting data...');
                $('#view-tid').text('Data');
            ...
            success: function (data) {
                ...
                $('#print_url').attr("href", "../print-encashment/" + data.encashment_data.id);
                ...
                if (data.encashment_data.status == 'pending') {
                    ...
                    var data_tax = parseInt(data_amount_req) * encashment_tax;
                    var total_claim = parseInt(data_amount_req) - parseInt(data_tax) - 20;
                    $('#view-aa').text('Pending');
                    $('#view-tax').text(addComma(data_tax) + 'PHP');
                ...
                $('#view-pfee').text(processing_fee+'PHP');
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (status == 'claimed' || status == 'claim') {
                ...
                setTimeout(function () {
                    window.location.href = '../encashment-voucher/' + id;
                }, 1500);

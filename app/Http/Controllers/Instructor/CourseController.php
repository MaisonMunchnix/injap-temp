...
class CourseController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'suggested_currency' => 'required|string|in:PHP,JPY',
        ...
        $courseData = [
            ...
            'suggested_currency' => $request->suggested_currency,
    ...
    public function update(Request $request, Course $course)
    {
        ...
        $request->validate([
            'title' => 'required|string|max:255',
            'description' => 'required|string',
            'suggested_currency' => 'nullable|string|in:PHP,JPY',
            'suggested_price' => 'required|numeric|min:0',
            'cover_photo' => 'nullable|image|mimes:jpeg,png,jpg,gif|max:2048',
        ]);
        ...
        $updateData = $request->only([
            ...
            'meeting_link', 'location', 'suggested_currency', 'suggested_price'
        ]);

        if ((float)$course->suggested_price != (float)$request->suggested_price || $course->suggested_currency != $request->suggested_currency) {

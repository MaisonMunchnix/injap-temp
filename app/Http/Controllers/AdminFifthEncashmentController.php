...
use App\Exports\FifthEncashmentExport;


class AdminFifthEncashmentController extends Controller
{
    /**
    ...
    public function adminViewEncashment($type, Request $request){
        ...
        if (!in_array($type, $valid_type)){
            ...
            $type=$type;
            if($type=='all'){
                    $get_encashment = DB::table('encashments')
                    ->join('users', 'users.id', '=', 'encashments.user_id')
                    ->join('user_infos', 'user_infos.user_id', '=', 'encashments.user_id')
                    ->select('users.username AS username','user_infos.first_name AS member_fname','user_infos.last_name AS member_lname','encashments.status AS encash_status','encashments.created_at AS encash_created','encashments.updated_at AS encash_updated','encashments.approved_date AS encash_appr_date','encashments.claimed_date AS encash_claimed_date','encashments.amount_requested AS amt_req','encashments.amount_approved AS amt_appr','encashments.id AS tid')
                    ->get();

                
                return view('admin.encashment.index',compact('get_encashment','type'));  
            }else{
                $get_encashment = DB::table('encashments')
                    ->join('users', 'users.id', '=', 'encashments.user_id')
                    ->join('user_infos', 'user_infos.user_id', '=', 'encashments.user_id')
                    ->select('users.username AS username','user_infos.first_name AS member_fname','user_infos.last_name AS member_lname','encashments.status AS encash_status','encashments.created_at AS encash_created','encashments.updated_at AS encash_updated','encashments.approved_date AS encash_appr_date','encashments.claimed_date AS encash_claimed_date','encashments.amount_requested AS amt_req','encashments.amount_approved AS amt_appr','encashments.id AS tid')
                    ->where('encashments.status',$type)
                    ->get();
                return view('admin.encashment.index',compact('get_encashment','type'));  
            }
        }
    ...
    public function adminViewEncashmentPaginate($type, Request $request){
        ...
        if (!in_array($type, $valid_type)){
            ...
            if($type == 'all'){
                if(empty($search)){
                    if($date_start){
                        $encashments = DB::table('fifth_encashments_view')
            ...
            if($request->button == "export"){
                return Excel::download(new FifthEncashmentExport($datecreate_start,$datecreate_end), 'Fifth Encashments-'. now() .'.xlsx');
            } else {
                return view('admin.encashment.fifth-encashment',compact('encashments','type','request'));  
            }
        }
	}

    public function adminViewVoucher($id){
        $get_encash = DB::table('fifth_encashments')
        ->join('users', 'users.id', '=', 'fifth_encashments.user_id')
        ->join('user_infos', 'user_infos.user_id', '=', 'fifth_encashments.user_id')
        ->leftJoin('banks', 'user_infos.bank_name', '=', 'banks.id')
        ->select('users.username AS username','user_infos.first_name AS member_fname','user_infos.last_name AS member_lname','user_infos.team_name','banks.name AS bank_name','user_infos.bank_account_number','user_infos.tin','fifth_encashments.*')
        ->where('fifth_encashments.id',$id)
        ->first();
        return view('admin.encashment.fifth_encashment_voucher',compact('get_encash'));  
    }
    ...
    public function allEncashments(Request $request){
        ...
        if($date_start){
            if($filter=='all'){
                $totalData = DB::table('fifth_encashments_view')
                    ->whereBetween('encash_created', array($datecreate_start . ' 00:00:00', $datecreate_end . ' 23:59:59'))
                    ->count();
            }else{
                $totalData = DB::table('fifth_encashments_view')
            ...
            if($filter=='all'){
                $totalData = DB::table('fifth_encashments_view')->count();
            }else{
                $totalData = DB::table('fifth_encashments_view')->where('encash_status',$filter)->count();
            }
        ...
        if(empty($request->input('search.value'))){
            ...
            if($filter=='all'){
                if($date_start){
                    ...
                    $totalFiltered = DB::table('fifth_encashments_view')
                        ->whereBetween('encash_created', array($datecreate_start . ' 00:00:00', $datecreate_end . ' 23:59:59'))
                        ->where('username','LIKE',"%{$search}%")
                        ->orWhere('member_fname', 'LIKE',"%{$search}%")
                        ->orWhere('member_lname', 'LIKE',"%{$search}%")
        ...
        if(!empty($encashments)){
            foreach($encashments as $encash){              
                ...
                if($encash->encash_status=='pending'){
                    ...
                    $action_data.="<a class='dropdown-item encash-view' href='#' data-id='$encash->tid'  data-target='#view_encashment_modal' data-toggle='modal'> View Details</a>";
                    $action_data.="<a class='dropdown-item' href='/staff/fifth-encashment-voucher/$encash->tid'> View receipt</a>";
                }
                $action_data.="</div>";
    ...
    public function getEncashmentData($id){
        $encashment_data=FifthEncashment::where('id',$id)->first();
        $user_data="";
        $user_process_data="";
    ...
    public function processEncashment(Request $request){
        ...
        if ($validator->fails()) {
            ...
            try {
                ...
                $encashment =FifthEncashment::find($request['id']);
                $encashment->process_by = $auth_id;
                $encashment->status = $request['status'];
                
                //$calc_tax = $request['amount_approved'] * 0.10;
                $calc_tax = 0;

                        ...
                        <div class="col-md-3">
                            ...
                            <div class="btn-group mr-1 mb-1">
                                ...
                                <div aria-labelledby="view_by" class="dropdown-menu">
                                    ...
                                    {{--<a class="dropdown-item filter-by @if($type=='approved') active @endif" href="{{ route('encashment-view','approved') }}" data-type="approved"> Approved</a>--}}
                                    <a class="dropdown-item filter-by @if($type=='claimed') active @endif" href="{{ route('encashment-view','claimed') }}" data-type="claimed"> Claimed</a>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-9">
                            <form action="{{ route('encashment-view',$type) }}" method="get" id="form-search">
                                @csrf
                                <div class="row">
                                    <div class="col-md-3 col-sm-6">
                                        <div class="form-group">
                                            <label>From</label>
                                            <input type="date" class="form-control date-picker" name="date_start" id="date_start" placeholder="Date" aria-label="Date" aria-describedby="basic-addon2" value="{{$request->input('date_start') ?? ''}}">
                                        </div>
                                    </div>
                                    <div class="col-md-3 col-sm-6">
                                        <div class="form-group">
                                            <label for="To">To</label>
                                            <input type="date" class="form-control date-picker" name="date_end" id="date_end" placeholder="Date" aria-label="Date" aria-describedby="basic-addon2" value="{{$request->input('date_end') ?? ''}}">
                                        </div>
                                    </div>
                                    <div class="col-md-3 col-sm-6">
                                        <div class="form-group">
                                            <label for="To">Username</label>
                                            <input type="search" class="form-control" name="search" id="search" placeholder="Search Username" aria-label="Date" aria-describedby="basic-addon2" value="{{$request->input('search') ?? ''}}">
                                        </div>
                                    </div>
                                    <div class="col-md-3 col-sm-6">
                                        <label>Search/Export</label><br>
                                        <div class="btn-group">
                                            <button class="btn btn-primary" id="search-button" name="button" type="submit" value="search"><i data-feather="search"></i> Search</button>
                                            <button class="btn btn-primary" id="export_button" name="button" value="export" type="submit"><i data-feather="download"></i> Export</button>
                                        </div>
                                    </div>
                                </div>
                            </form>
...
<script src="{{asset('js/admin/admin-encashment.js?v=1.1')}}"></script>
<script>
    var start = moment();
    var end = moment();

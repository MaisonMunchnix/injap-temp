...
class AdminEncashmentController extends Controller
{
        ...
        if (!in_array($type, $valid_type)){
            ...
            if($type=='all'){
                ...
                $get_encashment = DB::table('encashments')
                    ...
                    ->join('user_infos', 'user_infos.user_id', '=', 'encashments.user_id')
                    ->select('users.username AS username','user_infos.first_name AS member_fname','user_infos.last_name AS member_lname','encashments.status AS encash_status','encashments.created_at AS encash_created','encashments.updated_at AS encash_updated','encashments.approved_date AS encash_appr_date','encashments.claimed_date AS encash_claimed_date','encashments.amount_requested AS amt_req','encashments.amount_approved AS amt_appr','encashments.id AS tid')
                    ->where('encashments.status',$pass_type)
                    ->get();
                return view('admin.encashment.index',compact('get_encashment','type'));  
            }
        }
    ...
    public function adminViewEncashmentPaginate($type, Request $request){
        ...
        if (!in_array($type, $valid_type)){
            ...
            if($type=='all'){
                if(empty($search)){
                    if($date_start){
                        $encashments = DB::table('encashments_view')
            ...
            if($request->button == "export"){
                ...
                return view('admin.encashment.index-paginate',compact('encashments','type','request'));  
            }
        }
	}

    public function adminViewVoucher($id){
        $get_encash = DB::table('encashments')
        ->join('users', 'users.id', '=', 'encashments.user_id')
        ->join('user_infos', 'user_infos.user_id', '=', 'encashments.user_id')
        ->leftJoin('banks', 'user_infos.bank_name', '=', 'banks.id')
        ->select('users.username AS username','user_infos.first_name AS member_fname','user_infos.last_name AS member_lname','user_infos.team_name','banks.name AS bank_name','user_infos.bank_account_number','user_infos.tin','encashments.*')
        ->where('encashments.id',$id)
        ->first();
        return view('admin.encashment.encashment_voucher',compact('get_encash'));  
    }
    ...
    public function allEncashments(Request $request){
        ...
        if($date_start){
            if($filter=='all'){
                $totalData = DB::table('encashments_view')
                    ->whereBetween('encash_created', array($datecreate_start . ' 00:00:00', $datecreate_end . ' 23:59:59'))
                    ->count();
            }else{
                $totalData = DB::table('encashments_view')
            ...
            if($filter=='all'){
                $totalData = DB::table('encashments_view')->count();
            }else{
                $totalData = DB::table('encashments_view')->where('encash_status',$filter)->count();
            }
        ...
        if(empty($request->input('search.value'))){
            ...
            if($filter=='all'){
                if($date_start){
                    ...
                    $totalFiltered = DB::table('encashments_view')
                        ->whereBetween('encash_created', array($datecreate_start . ' 00:00:00', $datecreate_end . ' 23:59:59'))
                        ->where('username','LIKE',"%{$search}%")
                        ->orWhere('member_fname', 'LIKE',"%{$search}%")
                        ->orWhere('member_lname', 'LIKE',"%{$search}%")
    ...
    public function getEncashmentData($id){
        $encashment_data=Encashment::where('id',$id)->first();
        $user_data="";
        $user_process_data="";
    ...
    public function printEncashmentData($id){
        ...
        return view('admin.encashment.print_details',compact('encashment_data','user_data','user_process_data'));         
    }
    ...
    public function processEncashment(Request $request){
        ...
        if ($validator->fails()) {
            ...
            try {
                ...
                $encashment =Encashment::find($request['id']);

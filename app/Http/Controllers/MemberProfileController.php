...
class MemberProfileController extends Controller {
    ...
    public function viewProfile(){
        ...
        $user_data = DB::table('users')
            ...
            ->select('users.id AS u_id','users.username AS uname','users.created_at AS user_created_at','users.email','users.unilevel_rank as rank','user_infos.*','packages.type AS package_type')
        ...
        $ranking=['0'=>'No rank','1'=>'1st','2'=>'2nd','3'=>'3rd','4'=>'4th','5'=>'5th'];
        $total_income=Referral::select('amount')->where('user_id',$auth_id)->sum('amount');
        $total_referral=Network::select('sponsor_id')->where('sponsor_id',$auth_id)->count();
        
        return view('user.profile.index',compact('auth_id','user_data','province','city','total_income','total_referral','ranking'));           
    }
    ...
    public function updateMemberProfile(Request $request){
        ...
        if ($validator->fails()) {
            ...
            DB::beginTransaction();		
            try {
                $get_user_info=UserInfo::select('id')->where('user_id',$auth_id)->first();
                $user_info = UserInfo::find($get_user_info->id);
    ...
    public function updateAccountProfile(Request $request){
        ...
        if ($validator->fails()) {
            ...
            try {
				...
				$user_info->bank_account_number = $account_number;
				$user_info->tin = $tin;
				$user_info->save();	
	 ...
	 public function updateMemberPassword(Request $request){
        ...
        if ($validator->fails()) {
            ...
            try {
                ...
                $user = User::find($get_user_info->id);

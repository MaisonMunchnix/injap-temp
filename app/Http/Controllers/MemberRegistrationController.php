...
class MemberRegistrationController extends Controller
{
	...
	public function saveMember(Request $request){
		...
		if($valid_code == false){
			...
			try {
				...
				$checknet = DB::table("networks")
					->where("sponsor_id", "=", $upline_placement_id)
					->where("upline_placement_id", "=", $upline_placement_id)
                ...
                if($matchPoints >=1){
                    ...
                    for($i=0; $i < $matchPoints; $i++) {
                        ...
                        if($pv_pairs_count > $pvPoints_limiter){
							...
							$check_pairs = DB::table('networks')->select('user_id')->where('upline_placement_id', $upline_placement_id)->where("sponsor_id", "!=", $upline_placement_id)->get();
   ...
   public function Networks(){
		$networks = Network::select('user_id')->groupby('user_id')->get();
...
// 	}
	...
	public function checkUpline($username) {
      ...
      $networksCount = count($networksCount);
		...
		if ($networksCount == 0) {
			...
			$available_pos = DB::table('networks')->where('upline_placement_id', $upline_id)
			->groupBy("placement_position")
			->first();
